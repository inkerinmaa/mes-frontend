...
import type { Order, EnergyReportRow } from '../../types'
...
const rows      = ref<EnergyReportRow[]>([])
...
async function generateReport() {
  ...
  try {
    ...
    rows.value = await apiFetch<EnergyReportRow[]>(url)
...
const columns: TableColumn<EnergyReportRow>[] = [
  {
    accessorKey: 'orderNumber',
    header: () => t('reports.energy.columns.order'),
  ...
  {
    accessorKey: 'startTs',
    header: () => t('reports.energy.columns.start'),
    cell: ({ row }) => h('span', { class: 'text-sm tabular-nums' }, row.original.startTs.replace('T', ' '))
  },
  {
    accessorKey: 'durationH',
    header: () => t('reports.energy.columns.duration'),
    cell: ({ row }) => h('span', { class: 'text-sm tabular-nums' }, row.original.durationH.toFixed(1))
  },
  {
    accessorKey: 'totalGasM3',
    header: () => t('reports.energy.columns.gas'),
    cell: ({ row }) => h('span', { class: 'text-sm tabular-nums font-medium' }, row.original.totalGasM3.toFixed(1))
  },
  {
    accessorKey: 'totalElecKwh',
    header: () => t('reports.energy.columns.electricity'),
    cell: ({ row }) => h('span', { class: 'text-sm tabular-nums font-medium' }, row.original.totalElecKwh.toFixed(0))
  },
  {
    accessorKey: 'totalWaterM3',
    header: () => t('reports.energy.columns.water'),
    cell: ({ row }) => h('span', { class: 'text-sm tabular-nums' }, row.original.totalWaterM3.toFixed(2))
  },
]
...
function exportCsv() {
  ...
  a.href = url
  a.download = `energy-report-${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
...
const totals = computed(() => ({
  gas:   rows.value.reduce((s, r) => s + r.totalGasM3, 0),
  elec:  rows.value.reduce((s, r) => s + r.totalElecKwh, 0),
  water: rows.value.reduce((s, r) => s + r.totalWaterM3, 0),
}))
</script>
...
<template>
  <div class="p-6 space-y-6 shrink-0">
    ...
    <div v-if="generated && rows.length > 0" class="grid grid-cols-3 gap-4">
      <UPageCard>
        <template #title>
          <span class="text-sm text-muted">{{ t('reports.energy.cards.totalGas') }}</span>
        </template>
        <p class="text-2xl font-semibold tabular-nums">{{ totals.gas.toFixed(0) }} <span class="text-sm font-normal text-muted">m³</span></p>
      </UPageCard>
      <UPageCard>
        <template #title>
          <span class="text-sm text-muted">{{ t('reports.energy.cards.totalElectricity') }}</span>
        </template>
        <p class="text-2xl font-semibold tabular-nums">{{ totals.elec.toFixed(0) }} <span class="text-sm font-normal text-muted">kWh</span></p>
      </UPageCard>
      <UPageCard>
        <template #title>
          <span class="text-sm text-muted">{{ t('reports.energy.cards.totalWater') }}</span>
        </template>
        <p class="text-2xl font-semibold tabular-nums">{{ totals.water.toFixed(1) }} <span class="text-sm font-normal text-muted">m³</span></p>
      </UPageCard>
    </div>
